...
function getFallbackChatResponse(message) {
  ...
  if (hasAny("게시판", "글쓰기", "커뮤니티", "board")) {
    return {
      reply: "할머니, 게시판으로 안내해드릴게요. 가수 팬덤 공간에서 응원글, 자유글, 투표 인증글을 확인할 수 있어요.",
      action: "OPEN_BOARD",
    };
  }
  ...
  if (hasAny("설정", "언어", "글씨", "마이페이지", "settings", "language")) {
    return {
      reply: "할머니, 설정 화면에서 언어와 글자 크기, 개인정보 안내를 확인할 수 있어요.",
  ...
  if (hasAny("홈", "처음", "메인", "home")) {
    return {
      reply: "할머니, 홈 화면으로 이동해드릴게요. 좋아하는 가수와 명예의 전당을 바로 볼 수 있어요.",
      action: "OPEN_HOME",
    };
  }
  ...
  return {
    reply: "할머니, 지금은 기본 안내 모드로 답변하고 있어요. 게시판, 설정, 홈 이동이 필요하면 말해 주세요.",
    action: null,
  };
}
